...
// exposed by the Kuber webserver.
...
// from Kuber to the Panel.
type RemoteQueryConfiguration struct {
	// The amount of time in seconds that Kuber should allow for a request to the Panel API
	...
	// Kuber instance. A single request is initially made to the Panel to get this number
...
type SystemConfiguration struct {
	// The root directory where all of the kubectyl data is stored at.
	RootDirectory string `default:"/var/lib/kubectyl" yaml:"root_directory"`

	// Directory where logs for server installations and other kuber events are logged.
	...
	// If set to false Kuber will not attempt to write a log rotate configuration to the disk
	...
	AutoRestart bool `default:"true" yaml:"auto_restart"`
}
...
type CrashDetection struct {
	...
	// Determines if Kuber should detect a server that stops with a normal exit code of
	// "0" as being crashed if the process stopped without any Kuber interaction. E.g.
...
type Backups struct {
	...
	// CompressionLevel determines how much backups created by kuber should be compressed.
	//
...
type Configuration struct {
	...
	// Determines if kuber should be running in debug mode. This value is ignored
	// if the debug flag is passed through the command line arguments.
	Debug bool

	AppName string `default:"Kubectyl" json:"app_name" yaml:"app_name"`
	...
	// is only required by users running Kuber without SSL certificates and using internal IP
...
// EnableLogRotation writes a logrotate file for kuber to the system logrotate
...
func EnableLogRotation() error {
	if !_config.System.EnableLogRotate {
		log.Info("skipping log rotate configuration, disabled in kuber config file")
		return nil
	}
	...
	if _, err := os.Stat("/etc/logrotate.d/kuber"); err == nil || !os.IsNotExist(err) {
		return err
	}
	...
	// but there is not a file for kuber already. In that case, let us write a new file to
	// it so files can be rotated easily.
	f, err := os.Create("/etc/logrotate.d/kuber")
	if err != nil {
		return err
	}
	defer f.Close()

	t, err := template.New("logrotate").Parse(`{{.LogDirectory}}/kuber.log {
    size 10M
    compress
    delaycompress
    dateext
    ...
    notifempty
    postrotate
        /usr/bin/systemctl kill -s HUP kubequ.service >/dev/null 2>&1 || true
    endscript
}`)
	if err != nil {
		return err
...
func ConfigureTimezone() error {
	...
	if _config.System.Timezone == "" {
		...
		if err != nil {
			...
			// fall through to UTC to get Kuber booted at least.
